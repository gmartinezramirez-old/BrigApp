    ...
    private void lockedFeed(RoadData data) {
        ...
        for (RoadEntry entry : data) {
            ...
            if ("replace".equalsIgnoreCase(entry.getMode())) {
                ...
                } else if ("fire".equalsIgnoreCase(entry.getValueType())) {
                    double oldSpeed = carEncoder.getSpeed(edge.getFlags());
                        if (oldSpeed != value) {
                        updates++;
                        logger.info("Fire change at " + entry.getId() + " (" + point + "). Old: " + oldSpeed + ", new:" + value);
                        edge.setFlags(carEncoder.setSpeed(edge.getFlags(), value));
                } else {
                    throw new IllegalStateException("currently no other value type than 'speed' and 'fire' is supported");
                }
            } else {
